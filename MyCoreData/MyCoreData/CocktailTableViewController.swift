...
class CocktailTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    //**Begin Copy**
    //2) Add filter search vars
    
    var filteredTableData = [NSManagedObject]()
    var resultSearchController = UISearchController()
    var cocktailArray = [NSManagedObject]()
    ...
    func updateSearchResults(for searchController: UISearchController)
    {
        ...
        let searchPredicate = NSPredicate(format: "strong CONTAINS[c] %@", searchController.searchBar.text!)
        let array = (cocktailArray as NSArray).filtered(using: searchPredicate)
    ...
    func loaddb()
    {
        ...
        let fetchRequest:NSFetchRequest<NSFetchRequestResult> = NSFetchRequest(entityName: "Cocktail")
        
        do {
            let fetchedResults = try managedContext.fetch(fetchRequest) as? [NSManagedObject]
            if let results = fetchedResults {
                cocktailArray = results
        ...
        if (self.resultSearchController.isActive) {
            let cell =
                tableView.dequeueReusableCell(withIdentifier: "Cell")
                    as UITableViewCell!
            let drink = filteredTableData[(indexPath as NSIndexPath).row]
            cell?.textLabel?.text = drink.value(forKey: "strong") as! String?
            ...
            let cell =
                tableView.dequeueReusableCell(withIdentifier: "Cell")
                    as UITableViewCell!
            let drink = cocktailArray[(indexPath as NSIndexPath).row]
            cell?.textLabel?.text = drink.value(forKey: "strong") as! String?
            cell?.detailTextLabel?.text = ">>"
            return cell!
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            if (self.resultSearchController.isActive) {
                ...
                context.delete(cocktailArray[(indexPath as NSIndexPath).row])
            }
        ...
        if segue.identifier == "UpdateContacts" {
            if let destination = segue.destination as?
                ...
                if (self.resultSearchController.isActive) {
                    ...
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        let selectedDevice:NSManagedObject = cocktailArray[SelectIndex] as NSManagedObject
                        destination.contactdb = selectedDevice
                    }
                }
            }
